    ...
    : process{parent}
{
    ...
    connect(timeoutTimer, &QTimer::timeout, this, &process_24::slot_onTimeout);

    runtimeTimer = new QTimer();
    runtimeTimer->setInterval(100);
    connect(runtimeTimer, &QTimer::timeout, this, &process_24::slot_updateRuntime);
}

void process_24::setSerial(serial_port *serial)
{
    _serial = serial;
    QObject::connect(this, &process_24::sig_send_msg_to_serial,
                     _serial, &serial_port::slot_send_msg_to_serial,
                     Qt::QueuedConnection);
    QObject::connect(_serial, &serial_port::sig_serial_readyRead,
                     this, &process_24::slot_serial_readyRead,
                     Qt::QueuedConnection);
}
...
void process_24::slot_start(QMap<QString, QList<QVariant> > map, test_type type, test_auto auto_type, QString delay)
{
    ...
    QObject::connect(this, &process_24::sig_phase_changed, this, &process_24::slot_phase_changed, Qt::DirectConnection);
...
void process_24::test_connect_to_device()
{
    connectRetryCount = 0;
...
void process_24::test_send_para_to_device()
{
    QByteArray frame;
    frame.append(QByteArray::fromHex("22"));      // 字节数
    frame.append(QByteArray::fromHex("0C"));      // 六项交流电流实验
    frame.append(QByteArray::fromHex("FE14"));      // 暂时未知
...
void process_24::attemptConnect()
{
...
void process_24::frame_parse(QByteArray frame)
{
    QStringList results;
