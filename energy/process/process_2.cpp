...
{
    _serial = serial;
    QObject::connect(this, &process_2::sig_send_msg_to_serial,
                     _serial, &serial_port::slot_send_msg_to_serial,
                     Qt::QueuedConnection);
    QObject::connect(_serial, &serial_port::sig_serial_readyRead,
                     this, &process_2::slot_serial_readyRead,
                     Qt::QueuedConnection);
}

void process_2::slot_start(QMap<QString, QList<QVariant>> map, test_type type, test_auto auto_type, QString delay)
{
    if (isRunning()) return;
...
void process_2::test_send_para_to_device()
{
    ...
    frame.append(QByteArray::fromHex("881300"));              // 频率
