...
test_25::test_25(quint16 test_id, QWidget *parent)
    : test(test_id, parent)
{
...
void test_25::slot_frame_parse_result(const QStringList &result)
{
    ...
    if(type == "action"){
        item = _ui_025->ui->tb_down_2->item(get_result_index(portName), 1);
        if (!item) {
            item = new QTableWidgetItem();
            _ui_025->ui->tb_down_2->setItem(get_result_index(portName), 1, item);
        }
    }else{
        item = _ui_025->ui->tb_down_2->item(get_result_index(portName), 2);
        if (!item) {
            item = new QTableWidgetItem();
            _ui_025->ui->tb_down_2->setItem(get_result_index(portName), 2, item);
        }
    }
    item->setText(time);
}

void test_25::init_UI()
{
...
void test_25::init_chart_widget()
{
    QVBoxLayout* btn_layout = new QVBoxLayout(_ui_025->ui->btn_widget);
    ...
    QHBoxLayout* chart_layout = new QHBoxLayout(_ui_025->ui->chart_widget);
    _current = new ac_u_six_chart(_ui_025->tb_cl_values);
    QObject::connect(_chart_btn_fdq, &QToolButton::clicked, _current, &ac_u_six_chart::slot_onZoomOut);
    QObject::connect(_chart_btn_o, &QToolButton::clicked, _current, &ac_u_six_chart::slot_setShowGridCircles);
    QObject::connect(_chart_btn_x, &QToolButton::clicked, _current, &ac_u_six_chart::slot_setShowAxes);
    QObject::connect(_group, &QButtonGroup::idClicked, _current, &ac_u_six_chart::slot_onModeChanged);
    QObject::connect(_ui_025, &ui_025::sig_charts_refresh, _current, &ac_u_six_chart::slot_charts_refresh);
    chart_layout->addWidget(_current);
}

void test_25::init_state_widget()
{
