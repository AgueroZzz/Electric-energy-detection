...
test_12::test_12(quint16 test_id, QWidget *parent)
    : test(test_id, parent)
{
    init_UI();
...
void test_12::init_top_widget()
{
    ...
    _btn_error_in     = createToolButton(":/icon/icon/gz.svg", "按键触发故障");
    _btn_end_test     = createToolButton(":/icon/icon/stop.svg", "停止实验");
    _btn_vector       = createToolButton(":/icon/icon/vector.svg", "矢量图");
    ...
    btn_layout->addWidget(_btn_vector);
