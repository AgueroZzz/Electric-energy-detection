...
test_26::test_26(quint16 test_id, QWidget *parent)
    : test(test_id, parent)
{
...
void test_26::slot_frame_parse_result(const QStringList &result)
{
    ...
    if(type == "action"){
        item = _ui_026->ui->tb_down_1->item(get_result_index(portName), 1);
        if (!item) {
            item = new QTableWidgetItem();
            _ui_026->ui->tb_down_1->setItem(get_result_index(portName), 1, item);
        }
    }else{
        item = _ui_026->ui->tb_down_1->item(get_result_index(portName), 2);
        if (!item) {
            item = new QTableWidgetItem();
            _ui_026->ui->tb_down_1->setItem(get_result_index(portName), 2, item);
        }
    }
    item->setText(time);
}

void test_26::init_UI()
{
...
void test_26::init_top_widget()
{
    ...
    QObject::connect(this, &test_26::sig_test_start, this, &test_26::slot_test_start, Qt::DirectConnection);
    QObject::connect(this, &test_26::sig_test_stop, this, &test_26::slot_test_stop, Qt::DirectConnection);
...
void test_26::init_chart_widget()
{
    QVBoxLayout* btn_layout = new QVBoxLayout(_ui_026->ui->btn_widget);
    ...
    QHBoxLayout* chart_layout = new QHBoxLayout(_ui_026->ui->chart_widget);
    _chart = new ac_i_u_twelve_chart(_ui_026->tb_cl_values);
    QObject::connect(_chart_btn_fdq, &QToolButton::clicked, _chart, &ac_i_u_twelve_chart::slot_onZoomOut);
    QObject::connect(_chart_btn_o, &QToolButton::clicked, _chart, &ac_i_u_twelve_chart::slot_setShowGridCircles);
    QObject::connect(_chart_btn_x, &QToolButton::clicked, _chart, &ac_i_u_twelve_chart::slot_setShowAxes);
    QObject::connect(_group, &QButtonGroup::idClicked, _chart, &ac_i_u_twelve_chart::slot_onModeChanged);
    QObject::connect(_ui_026, &ui_026::sig_charts_refresh, _chart, &ac_i_u_twelve_chart::slot_charts_refresh);
    chart_layout->addWidget(_chart);
}

void test_26::init_state_widget()
{
...
void test_26::slot_test_start()
{
    ...
    _process_26 = new process_26(this);
    _process_26->setSerial(_serialPort.data());
    connect_test_to_process(this, _process_26);
    if(!_process_26) return;
    _process_26->slot_start(_ui_026->tb_cl_values,
                            get_test_type(_ui_026->leftGroup->checkedButton()->text()),
                            get_test_auto(_ui_026->rightGroup->checkedButton()->text()),
                            _ui_026->ui->le_delay->text());
}

void test_26::slot_test_stop()
{
    if (_process_26) {
